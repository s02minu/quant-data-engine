...
def test_bronze_path_has_hive_partition_layout():
    cfg = StreamConfig()
    when = datetime(2026, 7, 23, 14, 3, 12, 123000, tzinfo=timezone.utc)
...
def test_bronze_path_is_unique_per_flush():
    cfg = StreamConfig()
    a = bronze_path(cfg, "trades", "BTCUSDT", datetime(2026, 7, 23, 14, 3, 12, 100000, tzinfo=timezone.utc))
    b = bronze_path(cfg, "trades", "BTCUSDT", datetime(2026, 7, 23, 14, 3, 12, 200000, tzinfo=timezone.utc))
    assert a != b
